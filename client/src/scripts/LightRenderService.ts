...
export class LightRenderService {
  ...
  render(character: Player, timestampDiff: number, isHiding: boolean, nbMsPerCycle: number) {
    ...
    var daylightFactor = (character.entity.pv <= 0 || character.role == Role.Bad) ? 2 : Math.sin(timestampDiff * 2 * Math.PI / nbMsPerCycle - Math.PI / 2) + 1; // Day cycle
